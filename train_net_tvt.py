...
def main (config):
  ...
  for foldername in folders:
    ...
    results = [pool.apply_async(addFiles, args=(file, DIR, foldername, i, PREAUG_DIM)) for i, file in enumerate(files)]
...
# Split into training and rest sets
  X_train, X_rest, y_train, y_rest = sklearn.cross_validation.train_test_split(
      features,
      label,
      random_state=SEED2,
      train_size=T_RATIO,
...
# Split into validation and testing sets
  tempRatio = V_RATIO / (1 - T_RATIO)
  X_valid, X_test, y_valid, y_test = sklearn.cross_validation.train_test_split(
      X_rest,
      y_rest,
      random_state=SEED2,
      train_size=tempRatio,
      stratify=y_rest,
  )
...
#   use dropout
# - same interface as previous the previous function
  test_fn = theano.function(inputs=[l_in.input_var, target_vector],
                             outputs=[deterministic_loss, deterministic_out])
...
# Early stopping configuration
  patience = 30  # Do this many epochs regardless
  patience_increase = 5     # wait this much longer for a new best
  ...
  for epoch_num in range(num_epochs):
      ...
      predicted_classes = np.argmax(probabilities, axis=1)
      # calculate accuracy for this epoch
      valid_accuracy = sklearn.metrics.accuracy_score(y_valid, predicted_classes)

      total_time = time.time() - start_time
      print("Epoch: %d, train_loss=%f, train_accuracy=%f, valid_loss=%f, valid_accuracy=%f, time=%fs"
            % (epoch_num + 1, train_loss, train_accuracy, valid_loss, valid_accuracy, total_time))
      ...
      if patience <= epoch_num:
        if patience_increase <= count:
          break
  print("network training complete")

  print("----------calculating test accuracy----------")
  # calculate test accuracy after the network is trained
  ...
  for batch_num in range(num_batches_test):
    ...
    loss, probabilities_batch = test_fn(X_batch, y_batch)
  ...
  print("test_loss=%f, test_accuracy=%f"
